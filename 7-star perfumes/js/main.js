...
async function fetchProducts() {
    const CACHE_KEY = 'ssp_products_v3';
    ...
    try {
        ...
        if (shopSnap.exists()) {
            ...
            if (shopData.whatsapp) SHOP.whatsapp = shopData.whatsapp;
        }
...
function renderProducts() {
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map((p, i) => `
        <div class="product-card" data-aos="fade-up" data-aos-delay="${(i % 4) * 100}" onclick="window.openModal('${p.id}')">
            <div class="card-img-wrap">
                ${p.tag ? `<span class="card-tag">${p.tag}</span>` : ''}
                ${!p.available ? '<div class="card-out">OUT OF STOCK</div>' : ''}
                <img src="${getProductImage(p)}" alt="${p.name}" loading="lazy" onerror="this.src='logo.jpg'">
            </div>
            <div class="card-body">
...
function updateCartBadge() {
    ...
    const badges = ['cart-badge', 'cart-fab-badge', 'cart-count-label'];
    badges.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.textContent = total;
            if (id !== 'cart-count-label') el.classList.toggle('show', total > 0);
        }
    });
    
    document.getElementById('cart-fab').classList.toggle('show', total > 0);
}
...
function buildCartWAMessage(name, phone, items, total) {
    const itemsList = items.map((it, i) => `${i+1}. *${it.name}* (${it.size}) x ${it.qty} — ₹${it.price * it.qty}`).join('\n');
    return `┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
  🛒 *NEW WEBSITE ORDER* 🛍️
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
...
🔗 *Shop Online:* https://sevenstarperfume.com
-----------------------------
📍 Seven Star Perfume, Ahmedabad`;
}

function buildSingleProductWA(p, size) {
    const imgUrl = getProductImage(p);
    // Include full URL if image is relative for WhatsApp preview
    const finalImg = imgUrl.startsWith('http') ? imgUrl : `${window.location.origin}/${imgUrl}`;
...
-----------------------------
🛒 Ordered via sevenstarperfume.com
📍 Shop: Ahmedabad`;
}
